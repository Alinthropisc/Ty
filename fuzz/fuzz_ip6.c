...
void addfuzz(char **fuzzbuf, int *fuzzbuf_sz, char *fuzztype) {
  size_t need = strlen(*fuzzbuf) + strlen(fuzztype) + 1;
  if ((int)need > *fuzzbuf_sz) {
    while (*fuzzbuf_sz < (int)need)
      *fuzzbuf_sz *= 2;
    char *tmp = realloc(*fuzzbuf, *fuzzbuf_sz);
    if (tmp == NULL) {
      fprintf(stderr, "addfuzz: out of memory\n");
      exit(1);
    }
    *fuzzbuf = tmp;
  }
