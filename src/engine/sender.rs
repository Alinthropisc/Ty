...
pub fn eui64_ll_pub(mac: &[u8; 6]) -> [u8; 16] {
...
async fn flood_loop<F>(max_packets: u64, rate_pps: u64, stats: Arc<Stats>, send_fn: F) -> Result<()>
where
    F: Fn() -> Result<()> + Send + Sync + 'static,
{
    ...
    loop {
        if max_packets > 0 && stats.sent.load(std::sync::atomic::Ordering::Relaxed) >= max_packets {
            break;
        }
...
pub async fn flood_router(cfg: FloodConfig, stats: Arc<Stats>) -> Result<()> {
    let iface = CString::new(cfg.interface.clone()).context("interface name contains null byte")?;
    ...
    let (dst_raw, dstmac_raw, ip6_raw) = unsafe {
        ...
        if dst.is_null() {
            bail!("ty_resolve6(ff02::1) failed");
        }
...
fn send_one_ns(
    ...
    if let Some(t) = target_raw {
        unsafe {
            std::ptr::copy_nonoverlapping(t as *const u8, ns_buf.as_mut_ptr(), 16);
        }
    } else {
...
fn send_one_na(
    ...
    if let Some(t) = target_raw {
        unsafe {
            std::ptr::copy_nonoverlapping(t as *const u8, na_buf.as_mut_ptr(), 16);
        }
    } else {
...
fn send_one_dhcp6(iface: CString) -> Result<()> {
    ...
    payload[4] = 0;
    payload[5] = 1; // option-code = 1 (Client ID)
    payload[6] = 0;
    payload[7] = 10; // option-len  = 10
    payload[8] = 0;
    payload[9] = 3; // DUID-type = 3 (LL)
    payload[10] = 0;
    payload[11] = 1; // hw-type = 1 (Ethernet)
    for b in &mut payload[12..18] {
        *b = fastrand::u8(..);
    }
...
pub async fn flood_rs(cfg: RsConfig, stats: Arc<Stats>) -> Result<()> {
    ...
    let (dst_raw, dstmac_raw) = unsafe {
        ...
        if dst.is_null() {
            bail!("ty_resolve6(ff02::2) failed");
        }
        let dstmac = ffi::ty_get_multicast_mac(dst);
        (dst as usize, dstmac as usize)
    };
