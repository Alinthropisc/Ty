...
impl NetworkReport {
    ...
    pub fn print(&self) {
        ...
        if self.routers.is_empty() {
            ...
            for r in &self.routers {
                println!(
                    ...
                    r.src_ip, r.lifetime_sec,
                    r.managed as u8, r.other as u8,
        ...
        println!(
            "  {}",
            if self.raguard_likely { "likely present — test bypass techniques" }
            else { "not detected" }
        );
        println!();
    ...
    pub fn print_json(&self) {
        ...
        let recs_json: String = self.recommendations.iter().map(|r| {
            format!(
        ...
        println!(
            ...
            self.interface, self.capture_secs,
            self.ra_packets, self.dhcp6_packets, self.ns_packets,
            self.addr_mode.as_str(), self.raguard_likely,
            routers_json, recs_json,
        );
    }
}
...
pub async fn analyze_network(
    ...
    let (ra_count, dhcp6_count, ns_count) = task::spawn_blocking(move || {
        capture_counts(iface_clone, duration_secs)
    })
    .await
    ...
    let routers: Vec<RouterObservation> = (0..ra_count.min(8)).map(|i| RouterObservation {
    ...
    let recommendations = build_recommendations(
        &routers, &addr_mode, raguard_likely, &interface,
    );
